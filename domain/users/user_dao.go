...
import (
	"fmt"
	"github.com/andrestor2/bookstore_users-api/datasources/mysql/users_db"
	"github.com/andrestor2/bookstore_users-api/logger"
	"github.com/andrestor2/bookstore_users-api/utils/errors"
	"github.com/andrestor2/bookstore_users-api/utils/mysql_utils"
	"strings"
)
...
func (user *User) FindByStatus(status string) ([]User, *errors.RestErr) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); err != nil {
			logger.Error("error when scan rows into user struct", err)
			return nil, errors.NewInternalServerError("database error")
		}
...
func (user *User) FindByEmailAndPassword() *errors.RestErr {
	...
	if err != nil {
		logger.Error("error when trying to prepare get by email and password statement", err)
		return errors.NewInternalServerError("database error")
	}
	defer stmt.Close()
	...
	if getErr := result.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); getErr != nil {
		...
		logger.Error("error when trying to prepare get by email and password statement", getErr)
		return errors.NewInternalServerError("database error")
	}
	return nil
}
